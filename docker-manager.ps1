...
function Wait-ForRedis {
    ...
    for ($i = 1; $i -le 30; $i++) {
        $pong = docker exec redis-demo redis-cli ping 2>$null
        if ($pong -eq "PONG") {
    ...
    docker logs redis-demo --tail 20
    return $false
}
...
function Wait-ForRabbitMQ {
    ...
    for ($i = 1; $i -le 60; $i++) {
        $status = docker exec rabbitmq-demo rabbitmqctl status 2>$null
        if ($status -match "Node") {
            Write-Success "RabbitMQ is ready"
            return $true
        }
        if ($i % 10 -eq 0) { Write-Info "Still waiting for RabbitMQ... ($i/60)" }
        Start-Sleep -Seconds 1
    }
    Write-Error "RabbitMQ failed to start"
    docker logs rabbitmq-demo --tail 20
    return $false
}
...
function Start-Containers {
    ...
    # Wait for containers
    Write-Step "Wait for containers ready" -Color Yellow
    $mysqlOk = Wait-ForMySQL
    $redisOk = Wait-ForRedis
    $rabbitOk = Wait-ForRabbitMQ

    if (-not ($mysqlOk -and $redisOk -and $rabbitOk)) {
        Write-Error "Some containers failed to start. Exiting."
        return
    }

    # Initialize database
    Write-Step "Initialize database" -Color Yellow
    docker exec mysql-demo mysql -u root -p123456 test_db -e "
    ...
    Write-Host "API: http://localhost:8080/api/user/list" -ForegroundColor Green
